...
if [[ "${COMPILER_TYPES}" =~ "clang" ]]; then
        git clone https://android.googlesource.com/platform/prebuilts/clang/host/linux-x86 -b pie-release --depth=1  "${KERNELDIR}/clang"
	...
	COMPILER_STRING='google clang 6'
        COMPILER_TYPE='google clang 6'
else
