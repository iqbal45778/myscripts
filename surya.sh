...
makekernel() {
	...
	CROSS_COMPILE_COMPAT="${KERNELDIR}/gcc32/bin/arm-none-linux-gnueabihf-"
	    ...
	    make -j$(nproc --all) O=out ARCH=arm64 CROSS_COMPILE=aarch64-elf- CROSS_COMPILE_COMPAT=arm-eabi- LD=${KERNELDIR}/gcc/bin/aarch64-elf-ld AR=aarch64-elf-ar AS=aarch64-elf-as NM=aarch64-elf-nm OBJDUMP=aarch64-elf-objdump OBJCOPY=aarch64-elf-objcopy CC=aarch64-elf-gcc
    fi
